    ...
    ui(new Ui::UDPSenderWidget)
{
    ...
    ServerudpSocket->abort(); //释放为UDP绑定的端口
    ...
    connect(m_timersend,SIGNAL(timeout()),this,SLOT(timersend_slot()));
...
void UDPSenderWidget::on_ConnetBtn_clicked()
{
    ...
    else if(ui->ConnetBtn->text()==tr("连接"))
    {
        ...
        if(ret>=0)
        {
            if(!m_timersend->isActive())
            {
                m_timersend->setInterval(ui->spinBox_interval->value());
                isSend=true;
                //m_timersend->start();
            }
...
void UDPSenderWidget::VehicleStatus_data(const autonavigation::VehicleStatus::ConstPtr &gp)
{
    ...
    if((m_current_gps!=m_gp->gps_millisecond)&&(m_current_gps!=0)&&isSend)
    {
        if(!m_timersend->isActive())
        {
            m_timersend->start();
    ...
    else if((m_current_gps==0)&&isSend)
    {
        if(!m_timersend->isActive())
        {
            m_timersend->start();
...
//    long gps_ms=m_gp->gps_millisecond;
//    qDebug()<<"m_gp->gps_millisecond:"<<gps_ms;
 ...
 ***************************************************************/
/**
 * @brief UDPSenderWidget::send_realVehicleStatus
 */
void UDPSenderWidget::send_realVehicleStatus()
{
    ...
    //Time
    qDebug()<<"m_gp->gps_week:"<<m_gp->gps_week;
    long ms=m_gp->gps_millisecond;
    qDebug()<<"m_gp->gps_millisecond:"<<ms;
    if(ms==m_current_gps)
    {
        m_counter++;
    }
...
//    date_day=realdate.toJulianDay()-date_2000.toJulianDay();
    ...
    QDate realDate=QDate::currentDate();
    qDebug()<<"realDate:"<<realDate;
    QDate date_1980(1980, 1, 6);
    int date_day=realDate.toJulianDay()-date_1980.toJulianDay();
    m_kongFrameHead.DData=date_day;


    int day_ms=ms%(24*3600*1000);
    QTime gps_time=QTime::fromMSecsSinceStartOfDay(day_ms).addMSecs(8*3600*1000);     //添加为北京东8时区
    qDebug()<<"gps_time:"<<gps_time;

    m_kongFrameHead.DTimeHH=gps_time.hour();
    m_kongFrameHead.DTimeMM=gps_time.minute();
    m_kongFrameHead.DTimeSS=gps_time.second();
    m_kongFrameHead.DTimeMS=gps_time.msec();
    ...
    m_kongData.Lon=m_gp->longitude*10e5;         //0.000001?
    m_kongData.Lat=m_gp->latitude*10e5;          //0.000001?
    m_kongData.Alt=m_gp->height*100;             //VehicleStatus中height单位为m
    m_kongData.GVel=m_gp->vehicle_speed*100;     //速度
    m_kongData.Head=m_gp->azimuth*100;           //航向
