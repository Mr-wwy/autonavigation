...
//ConHCMap ConHCCameraThread::m_Map;
//int   ConHCCameraThread::m_SID=0;

ConHCCameraThread* ConHCCameraThread::m_conHC = NULL;

void ConHCCameraThread::yv12toYUV(char *outYuv, char *inYv12, int width, int height,int widthStep)
{
    int col,row;
    unsigned int Y,U,V;
    int tmp;
...
void  CALLBACK ConHCCameraThread::DecCBFun(int  nPort,char * pBuf,int nSize,FRAME_INFO * pFrameInfo, void* nReserved1,int nReserved2)
{
...
//        cv::putText(frametemp,_text,cv::Point(2, 15),cv::FONT_HERSHEY_SIMPLEX, 0.45,cv::Scalar(0,255,0,0),1);
...
//        ROS_INFO("height: %d, width: %d", msg_ptr->height, msg_ptr->width);
        ...
        std::cout<<"publish1:"<<"height:"<<msg_ptr->height<<" "<<"width:"<<msg_ptr->width<<std::endl;
        m_conHC->pub_img.publish(msg_ptr);
        //m_Map.value(m_SID)->pub_img.publish(msg_ptr);
...
void CALLBACK ConHCCameraThread::RealDataCallBack(LONG lRealHandle,DWORD dwDataType,BYTE *pBuffer,DWORD dwBufSize,void *pUser)
{
...
ConHCCameraThread::ConHCCameraThread(const char *deviceIp, const char *userName,const char *passWord, const char *topicName,
...
ConHCCameraThread::~ConHCCameraThread()
{   
    this->stop();
...
void ConHCCameraThread::setParameter(int32_t StreamType, int32_t VideoFrameRate, int32_t VideoBitRate)
{
...
int ConHCCameraThread::setConnInfo(const char* deviceIp, const char* userName, const char* passWord, const char* topicName)
{
    if (deviceIp[0] == 0)
    {
    ...
    //m_publisher = m_nodeHandle.advertise<autonavigation::VideoImage>(m_videoParam.topicName, 10);
...
void ConHCCameraThread::setStreamType(boost::int32_t c)
{
    m_videoParam.dwStreamType = c;
}

void ConHCCameraThread::setVideoFrameRate(boost::int32_t c)
{
    m_videoParam.dwVideoFrameRate = c;
}

void ConHCCameraThread::setVideoBitRate(boost::int32_t c)
{
    m_videoParam.dwVideoBitrate=c;
}

int ConHCCameraThread::connectHK()
{
//    ros::Rate r(1);
//    while (ros::ok())
//    {
//        //printf("try to connect camera ... \n");
//        qDebug()<<"try to connect camera ... ";
//        if(isstop)
//        {
//            //qDebug()<<"connect stop";
//            break;
...
int ConHCCameraThread::resetHKandconnectHK(int Stream_Type)
{    
...
void ConHCCameraThread::stop()
{
    isstop=true;

}

void ConHCCameraThread::startRealPlay()
{
    if(m_rpstartstopflag==0)
    {
        //设置回调数据流
...
void ConHCCameraThread::stopRealPlay()
{
...
void ConHCCameraThread::saveorstopRealPlay()
{
    if (m_rpstartstopflag == 0)
    {
