...
bool logitech::init()
{
    ...
    m_pLogitech->ConcatenonSignal=0;
...
int logitech::get_event_imitate_data(double *position)
{
    ...
    while (1)
    {
        ...
        //ROS_INFO("read %d bytes!, %d", nbytes, sizeof(event));
...
int logitech::processEvent(input_event *event,double *position)
{
    ...
    if (event->type==EV_ABS && event->code==axis_code)   //方向盘
    {
    ...
    else if(event->type==EV_KEY)
    {
       ...
       {
           if(event->code==710||event->code==300||event->code==301||event->code==302||event->code==303||event->code==704)
           {
               m_pLogitech->StickShift=6;
               m_pLogitech->SteeringWheel=temp_wheel;
               m_pLogitech->Brake=temp_break;
               m_pLogitech->Clutch=temp_Clutch;
               m_pLogitech->SasPedal=temp_SasPedal;
