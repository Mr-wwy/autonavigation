...
void PointWidget::initwidget()
{
    ...
    tableview_load=new QTableView(widtemptab1); //表格视图控件
    ...
    tablemodel_load->setHorizontalHeaderLabels(headerlabels);   //使用标签设置水平标题标签

    tableview_load->setModel(tablemodel_load);  //绑定数据模型

    tableview_load->horizontalHeader()->setSectionResizeMode(8,QHeaderView::Stretch);
    tableview_load->setEditTriggers(QAbstractItemView::NoEditTriggers);//设置表格只读，不能进行编辑
    tableview_load->horizontalHeader()->setStyleSheet("QHeaderView::section{background:skyblue;}");
    tableview_load->setAlternatingRowColors(true);  //隔行自动变色
