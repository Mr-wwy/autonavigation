    ...
    ui(new Ui::DrivingWidget)
{
    ...
    this->setFocusPolicy(Qt::StrongFocus);  //设置获得焦点的方式
...
void DrivingWidget::initialWidget()
{
    ...
    LEftfunction->setGeometry(g_nActScreenX-400,0,400,150);
    ...
    m_labelBrake->setGeometry(g_nActScreenX-265,g_nActScreenY-110,25,25);
    m_labelBrake->setText(tr("刹车"));

    m_labelThrottle->setGeometry(g_nActScreenX-185,g_nActScreenY-110,25,25);
    m_labelThrottle->setText(tr("油门"));
...
//    m_LE_millisecond->setValidator(rxValidator);
    ...
    QFont m_font2(tr("意大利体"),12,50);    //设置字体
...
void DrivingWidget::action_tip_slot(bool)
{
    ...
    else
    {
        ...
        connect(m_operatehelp,SIGNAL(destroyed(QObject*)),this,SLOT(operatehelp_destoryed(QObject*)));  //提示被关闭就调用槽函数
        m_operatehelp->show();
...
void DrivingWidget::VehicleStatus_data(const autonavigation::VehicleStatus::ConstPtr &gp)
{
    ...
    m_taskmode->change_taskmode(m_mode,m_controlstatus);    //车辆模式

    //m_wheel_r->wheel_rotate(gp->steer*540/3000);
    m_indicatorBrake_r->setValue(gp->brake);        //刹车油门信息
    m_indicatorThrottle_r->setValue(gp->throttle);

    SPeed->change_Speed(abs(gp->vehicle_speed),gp->shift);  //速度档位
    TAchometer->change_tachometer(gp->engine_speed,gp->engine_status,gp->handbrake);    //转速

    //FPs->change_fps(gp->time_delay);
    //qDebug()<<"time_delay:"<<gp->time_delay;

    LEftfunction->change_LeftFunction(gp->travel_distance,gp->remain_time);   //靶车跑的距离
    RIghtfunction->change_right(gp->gps_week,gp->gps_millisecond,gp->latitude,gp->longitude,
                                gp->gaussX,gp->gaussY,gp->azimuth,gp->pitch,gp->roll);  //高斯坐标等

//    qDebug()<<"lati:"<<gp->latitude;
//    qDebug()<<"longti:"<<gp->longitude;

    m_leftdisplay->setRecordStatusAndEnable(gp->reserve3,gp->enable);   //录路状态

    OIltable->change_oiltable(gp->fuel);    //剩余油量
    WAtertemperature->change_watertem(gp->water_temperature);   //水温
...
void DrivingWidget::readSettings()
{
    ...
    Fx=settings.value("Driving/Fx",539.4193).toDouble();    //如果配置文件中Driving/Fx的值为空，则使用539.4193
