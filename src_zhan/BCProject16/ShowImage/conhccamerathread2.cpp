...
ConHCCameraThread2* ConHCCameraThread2::m_conHC2 = NULL;

void ConHCCameraThread2::yv12toYUV(char *outYuv, char *inYv12, int width, int height,int widthStep)
{
    int col,row;
    unsigned int Y,U,V;
    int tmp;
...
void  CALLBACK ConHCCameraThread2::DecCBFun(int  nPort,char * pBuf,int nSize,FRAME_INFO * pFrameInfo, void* nReserved1,int nReserved2)
{
...
//        m_gV->m_publisher.publish(pVideoImage);
        ...
        m_conHC2->pub_img.publish(msg_ptr);
...
void CALLBACK ConHCCameraThread2::RealDataCallBack(LONG lRealHandle,DWORD dwDataType,BYTE *pBuffer,DWORD dwBufSize,void *pUser)
{
    ...
    switch (dwDataType)
    {
        case NET_DVR_SYSHEAD:
            ...
            //是指解码回调函数，只解码不显示
            if (!PlayM4_SetDecCallBack(nPort2, DecCBFun))
            {
                dRet=PlayM4_GetLastError(nPort2);
                break;
            }
            ...
            //打开视频解码
            if (!PlayM4_Play(nPort2, hPlayWnd2))
            {
                dRet=PlayM4_GetLastError(nPort2);
                break;
            }
...
ConHCCameraThread2::ConHCCameraThread2(const char *deviceIp, const char *userName,const char *passWord, const char *topicName,
...
ConHCCameraThread2::~ConHCCameraThread2()
{
    this->stop();
...
void ConHCCameraThread2::setParameter(int32_t StreamType, int32_t VideoFrameRate, int32_t VideoBitRate)
{
...
int ConHCCameraThread2::setConnInfo(const char* deviceIp, const char* userName, const char* passWord, const char* topicName)
{
    if (deviceIp[0] == 0)
    {
...
int ConHCCameraThread2::process()
{
...
void ConHCCameraThread2::setStreamType(boost::int32_t c)
{
    m_videoParam.dwStreamType = c;
}

void ConHCCameraThread2::setVideoFrameRate(boost::int32_t c)
{
    m_videoParam.dwVideoFrameRate = c;
}

void ConHCCameraThread2::setVideoBitRate(boost::int32_t c)
{
    m_videoParam.dwVideoBitrate=c;
}

int ConHCCameraThread2::connectHK()
{
//    ros::Rate r(1);
//    while (ros::ok())
//    {
...
int ConHCCameraThread2::resetHKandconnectHK(int Stream_Type)
{
...
void ConHCCameraThread2::startRealPlay()
{
    if(m_rpstartstopflag==0)
    {
        //设置回调数据流
...
void ConHCCameraThread2::stopRealPlay()
{
...
void ConHCCameraThread2::saveorstopRealPlay()
{
    if (m_rpstartstopflag == 0)
    {
