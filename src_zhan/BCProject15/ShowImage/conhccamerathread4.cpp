...
ConHCCameraThread4* ConHCCameraThread4::m_conHC4 = NULL;

void ConHCCameraThread4::yv12toYUV(char *outYuv, char *inYv12, int width, int height,int widthStep)
{
    int col,row;
    unsigned int Y,U,V;
    int tmp;
...
void  CALLBACK ConHCCameraThread4::DecCBFun(int  nPort,char * pBuf,int nSize,FRAME_INFO * pFrameInfo, void* nReserved1,int nReserved2)
{
...
//        m_gV->m_publisher.publish(pVideoImage);
        std::cout<<"publish_4:"<<"height:"<<msg_ptr->height<<" "<<"width:"<<msg_ptr->width<<std::endl;
        m_conHC4->pub_img.publish(msg_ptr);
...
void CALLBACK ConHCCameraThread4::RealDataCallBack(LONG lRealHandle,DWORD dwDataType,BYTE *pBuffer,DWORD dwBufSize,void *pUser)
{
    ...
    switch (dwDataType)
    {
        case NET_DVR_SYSHEAD:
            ...
            //是指解码回调函数，只解码不显示
            if (!PlayM4_SetDecCallBack(nPort4, DecCBFun))
            {
                dRet=PlayM4_GetLastError(nPort4);
                break;
            }
            ...
            //打开视频解码
            if (!PlayM4_Play(nPort4, hPlayWnd4))
            {
                dRet=PlayM4_GetLastError(nPort4);
                break;
            }
...
ConHCCameraThread4::ConHCCameraThread4(const char *deviceIp, const char *userName,const char *passWord, const char *topicName,
...
ConHCCameraThread4::~ConHCCameraThread4()
{
    this->stop();
...
void ConHCCameraThread4::setParameter(int32_t StreamType, int32_t VideoFrameRate, int32_t VideoBitRate)
{
...
int ConHCCameraThread4::setConnInfo(const char* deviceIp, const char* userName, const char* passWord, const char* topicName)
{
    if (deviceIp[0] == 0)
    {
...
int ConHCCameraThread4::process()
{
...
void ConHCCameraThread4::setStreamType(boost::int32_t c)
{
    m_videoParam.dwStreamType = c;
}

void ConHCCameraThread4::setVideoFrameRate(boost::int32_t c)
{
    m_videoParam.dwVideoFrameRate = c;
}

void ConHCCameraThread4::setVideoBitRate(boost::int32_t c)
{
    m_videoParam.dwVideoBitrate=c;
}

int ConHCCameraThread4::connectHK()
{
//    ros::Rate r(1);
//    while (ros::ok())
//    {
...
int ConHCCameraThread4::resetHKandconnectHK(int Stream_Type)
{
...
void ConHCCameraThread4::startRealPlay()
{
    if(m_rpstartstopflag==0)
    {
        //设置回调数据流
...
void ConHCCameraThread4::stopRealPlay()
{
...
void ConHCCameraThread4::saveorstopRealPlay()
{
    if (m_rpstartstopflag == 0)
    {
