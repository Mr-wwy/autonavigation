...
//    qDebug()<<"T:"<<T[0]<<T[1]<<T[2];
    ...
    m_light=0;                    //灯光
    m_reset=0;                    //复位
    m_reserve2=0;                 //record  0:不记录   1：开始记录  2:保存成功
    m_reserve3=0;
    ...
    qRegisterMetaType<autonavigation::SetT0::ConstPtr>("autonavigation::SetT0::ConstPtr");
    ...
    connect(&m_recvgpstimethread,SIGNAL(GpsTime_readed(autonavigation::GpsTime::ConstPtr)),this,SLOT(GpsTime_data(autonavigation::GpsTime::ConstPtr)));
    connect(&m_recvt0thread,SIGNAL(sett0_readed(autonavigation::SetT0::ConstPtr)),this,SLOT(T0_data(autonavigation::SetT0::ConstPtr)));
...
void DrivingWidget::initialWidget()
{
    ...
    connect(m_map,SIGNAL(modifyfilename_signals(QString)),this,SLOT(modifyfilename_slot(QString)));

    /*****************************排放界面控件位置*******************************/
    WAtertemperature->setGeometry(g_nActScreenX-840,g_nActScreenY-350,400,400);
    WAtertemperature->hide();

    LEftfunction->setGeometry(g_nActScreenX-500,10,450,130);
    LEftfunction->setDistance_guihua(m_save_distance);

    m_leftdisplay->setGeometry(10,150,330,150);

    RIghtfunction->setGeometry(g_nActScreenX-550,150,450,200);
...
//    m_gps_millisecond=gp->gps_millisecond;
    ...
    LEftfunction->setDistanceAndTime(gp->travel_distance,gp->remain_time);
    m_real_distance=gp->travel_distance;
...
//    qDebug()<<"longti:"<<gp->longitude;
    ...
    m_leftdisplay->setEnable(gp->enable);
    m_leftdisplay->setRecordStatus_status(gp->reserve3);
...
/*************************************槽函数-处理GpsTime数据*********************************************/
void DrivingWidget::GpsTime_data(const autonavigation::GpsTime::ConstPtr &gpstime)
{
    m_gps_millisecond=gpstime->gps_millisecond;
//    qDebug()<<"gps:"<<m_gps_millisecond;
...
//    QDateTime T0_time=QDateTime::fromMSecsSinceEpoch(time_1980+m_gps_millisecond);
//    qDebug()<<"time:"<<T0_time.toString("yyyy/MM/dd HH:mm:ss.zzz");

    if((m_gps_millisecond>=m_cmd_millisecond)&&(m_cmd_millisecond>0))
    {
        m_force_start=1;
    }
    sendControlCommand();
...
/*************************************槽函数-处理T0数据*********************************************/
void DrivingWidget::T0_data(const autonavigation::SetT0::ConstPtr &sett0)
{
    QString strBuffer;
    QDateTime time;
    strBuffer = GPSTIME_STR;
    ...
    long real_T0=sett0->cmd_millisecond+m_setdelaytime_t0;

    QDateTime T0_time=QDateTime::fromMSecsSinceEpoch(time_1980+real_T0);

    m_cmd_millisecond=real_T0;
    RIghtfunction->setTime_T0(QString("%1").arg(T0_time.toString("yyyy/MM/dd HH:mm:ss.zzz")));
...
//        }
        ...
        if(m_c_count==2)
        {
            m_save_distance=m_real_distance;
            LEftfunction->setDistance_guihua(m_save_distance);
        }
//        m_leftdisplay->setRecordStatus_count(m_c_count);
...
void DrivingWidget::change_taskmode(int mode)
{
    ...
    {
        m_setdelaytime=new SetTime(0,m_setdelaytime_t0);
        connect(m_setdelaytime,SIGNAL(settime_signals(int)),this,SLOT(settime_surebtn_clicked_slot(int)));
        m_setdelaytime->move(this->geometry().center()-m_setdelaytime->geometry().center());
        m_setdelaytime->show();
...
/***********************************槽函数-设置延迟时间（ms）**********************************************/
void DrivingWidget::settime_surebtn_clicked_slot(int _time)
{
    m_setdelaytime_t0=_time;
}
...
void DrivingWidget::gpsmillisecond_slot(QString gm)
{
    ...
    long real_T0=gm.toLong()+m_setdelaytime_t0;
...
/***********************************槽函数-修改局部路径文件名**********************************************/
void DrivingWidget::modifyfilename_slot(QString newfilename)
{
...
void DrivingWidget::readSettings()
{
    ...
    m_setdelaytime_t0=settings.value("Driving/SetTime",0).toInt();
    m_save_distance=settings.value("Driving/Distance",0).toInt();
...
void DrivingWidget::writeSettings()
{
    ...
    settings.setValue(tr("SetTime"),m_setdelaytime_t0);
    settings.setValue(tr("Distance"),m_save_distance);
